...
MIN_TRI_ANGLE_DEG = 15.0          # Minimum ray divergence to trust triangulated z
PHYSICS_STEPS_PER_FRAME = 10
ROBOTS = {0: "robot1_aruco_plate", 1: "robot2_aruco_plate"}
YAW_OFFSET_DEG = None

# Worst-case anomaly triggers for automatic dumping
WORST_ERR_XY_THRESH_MM = 20.0     # Save snapshot if horizontal error exceeds 20mm
WORST_ERR_Z_THRESH_MM = 15.0      # Save snapshot if z triangulation error exceeds 15mm
...
def measure(cam, corners, plate_z):
    ...
    # Reconstructed side length error against known 0.064m
...
def triangulate(obs):
    """Least-squares intersection point closest to all centre rays."""
...
def fuse(obs, yaw_offset_deg, plate_z):
    ...
    # If z is triangulated, full pose is (x, y, z_tri); otherwise default to planar plate_z
...
def main():
    ...
    # True plate top surface elevation in world frame
    ...
    win = "BotArena: v2 3D Pose & Camera Overlay Diagnostic Workbench"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(win, int(CAM_W * 2 * DISPLAY_SCALE), int(CAM_H * DISPLAY_SCALE) + 140)
    ...
    print("=" * 75)
    print(f" V2 TRACKER: 3D POSE + DUAL-CAMERA BENCHMARK (PLATE Z: {plate_z:.4f}m)")
    print(f" Snapshots on error spikes saved to: ./{DUMP_DIR}/")
    print("=" * 75)
    ...
    try:
        while True:
            ...
            # Ground truth 3D poses (center of ArUco plate top face)
            gt_3d = {
                m: np.array([
                    data.geom_xpos[g][0],
                    data.geom_xpos[g][1],
            ...
            for idx, cam in enumerate(cams):
                ...
                if ids is not None:
                    for c, mid in zip(corners, ids.flatten()):
                        ...
                        cv2.putText(
                            bgr,
                            f"R{mid} dS:{o.scale_err*100:+.1f}% inc:{o.incidence_deg:.0f}d",
                            (int(cp[0]), int(cp[1] - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.45,
                            col,
            ...
            for mid in ROBOTS:
                ...
                # Trigger automated anomaly snapshot
                now = time.time()
                is_anomaly = (
                ...
                if is_anomaly and (now - last_dump_time[mid] > 2.0):
                    ...
                    fname = f"{DUMP_DIR}/worst_R{mid}_f{frame:05d}_xy{int(err_xy)}mm{z_tag}_yaw{int(err_yaw)}d.png"
                    cv2.imwrite(fname, combined_full)
                    print(f"\n[!] ANOMALY SNAPSHOT: {fname}")
            ...
            for mid in ROBOTS:
                ...
                if mid in fused:
                    ...
                    z_str = f"Z: {e['pose_3d'][2]:.4f}m" if e["z_tri"] is not None else f"Z: {plate_z:.4f}m (Plane)"
                    ...
                    l2 = f"Gate Drops: {st['scale_gate_rejections']} | Total Lost Frames: {st['dropouts']}"

                l3 = (
                    f"WORST PEAKS -> MaxXY: {st['max_xy_err_mm']:.1f}mm | "
                    f"MaxZ: {st['max_z_err_mm']:.1f}mm | MaxYaw: {st['max_yaw_err_deg']:.1f}d"
                )
                l4 = (
                    f"ScaleDev: {st['max_scale_err_pct']:.1f}% | IncAngle: {st['max_incidence_deg']:.1f}d | "
                    f"Uptime: {((frame - st['dropouts']) / max(frame, 1))*100:.1f}%"
                )
        ...
        print("\n" + "=" * 75)
        print(" FINAL 3D POSE DIAGNOSTIC RUN SUMMARY")
        print("=" * 75)
        for mid, st in worst_case.items():
            ...
            print(f"  Worst Marker Cell    : {st['min_cell_px']:.2f} px/bit")
            print(f"  Worst Scale Dev      : {st['max_scale_err_pct']:.2f} % (Gate: 8.0%)")
            print(f"  Worst Incidence Angle: {st['max_incidence_deg']:.1f} deg")
            print(f"  Scale Gate Drops     : {st['scale_gate_rejections']}")
            print("-" * 75)
