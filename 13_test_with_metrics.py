"""
Dual Slanted-Camera ArUco Tracker (v2 Stress Benchmark + Live Telemetry Plots)
...
- Synchronous live Matplotlib dashboard (XY arena trajectory & rolling error in mm).
...
DISPLAY_SCALE = 0.65                  # Fitted for 1920x1080 / 1920x1200 screens
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Matplotlib Live Telemetry Setup
    # -------------------------------------------------------------------------
    plt.ion()
    fig, (ax_traj, ax_err) = plt.subplots(1, 2, figsize=(11, 5))
    fig.canvas.manager.set_window_title("BotArena: Live Telemetry & Error Monitor")
    ...
    (line_est_r0,) = ax_traj.plot([], [], "c--", lw=1.2, label="R0 v2 Estimated")
    (line_gt_r1,) = ax_traj.plot([], [], "m-", lw=1.8, label="R1 Ground Truth")
    (line_est_r1,) = ax_traj.plot([], [], "y--", lw=1.2, label="R1 v2 Estimated")
    ax_traj.legend(loc="upper right", fontsize=8)

    ax_err.set_xlim(0, 10)
    ax_err.set_ylim(0, 10)  # v2 planar tracking error scale
    ax_err.set_title("Absolute Tracking Error")
    ax_err.set_xlabel("Time (s)")
    ax_err.set_ylabel("Error (mm)")
    ax_err.grid(True, linestyle="--", alpha=0.5)

    (line_err_r0,) = ax_err.plot([], [], "c-", lw=1.5, label="R0 Error")
    (line_err_r1,) = ax_err.plot([], [], "m-", lw=1.5, label="R1 Error")
    ...
    try:
        while True:
            ...
            for mid in ROBOTS:
                ...
                # Record live data to telemetry buffers
                traj[mid]["times"].append(sim_time)
                traj[mid]["gt_x"].append(gt_3d[mid][0])
                traj[mid]["gt_y"].append(gt_3d[mid][1])
                traj[mid]["est_x"].append(est["xy"][0])
                traj[mid]["est_y"].append(est["xy"][1])
                traj[mid]["err"].append(err_xy)
                ...
                # Trigger automatic anomaly dump
                now = time.time()
                is_anomaly = (
            ...
            # Refresh Matplotlib Plots (every 5 visual frames)
            if frame % 5 == 0:
                ...
                ax_err.set_xlim(max(0, sim_time - 10), sim_time + 0.5)
